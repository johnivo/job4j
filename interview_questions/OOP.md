...
Это создание конструкторов с другим набором аргументов (как по количеству, так и по порядку следования).

Перегрузка конструктора позволяет установить разнличные способы начальной инициализации объектов класса.
...
Это метод, для работы которого необходим объект. Т.е. он принадлежит объекту класса и может быть вызван с помощью ссылки на данный объект.
...
1. Объявление переменной с именем и типом объекта `Book b;`

2. Инстационирование (создание) и инициализация объекта `new Book();` 
...
т.е. объект создается с помощью ключевого слова `new`, которое вызывает конструктор класса, 
...
Это функция (метод), которая может быть переопределена наследником.
...
Это наличие в классе нескольких методов с одним именем, но разными наборами параметров.
...
Используется для обращения к абстрактному методу и из него получить информацию о конкретной реализации.
```
...
+ **Статические**
```
StaticInnerClass public class OuterClass { 
...
+ **Нестатические** вложенные в другой класс - это **внутренние классы**.
    ...
    ```
    InnerClass public class OuterClass { 
         //определены внутри класса
    ...
    + локальные классы
    ```
    LocalClass public class OuterClass { 
...
Это класс, который сразу создается и реализуется (без имени класса) от суперкласса или интерфейса.
